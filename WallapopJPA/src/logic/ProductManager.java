...
//		manager.deleteAllByIdr();
	...
	public List<Product> findAdvanced(Product producto) {
		...
		try {
			Query query = em.createNamedQuery("SELECT c FROM Product c WHERE c.name = :varName OR c.description LIKE :varDesc OR c.category.id = :varCategoryId OR c.user.name = :varUserName OR c.user.city = :varUserCity", Product.class);
